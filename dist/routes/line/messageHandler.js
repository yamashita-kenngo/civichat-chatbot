...
module.exports = async (event) => {
    ...
    switch (event.type) {
        case "message":
            if (event.message.type === "text") {
                if (event.message.text === "Civichatをはじめる") {
                            ...
                            actions: [
                                {
                                    ...
                                    linkUri: "https://twitter.com/civichat/status/1435516003140583428?openExternalBrowser=1",
                                },
                                {
                                    type: "uri",
                                    area: {
                                    ...
                                    linkUri: "https://www.instagram.com/p/CTpGWcJBg0N?openExternalBrowser=1",
                                },
                            ],
                        },
                    ];
                    ...
                    // ユーザーのセッション取得
                    const userSession = sessions[event.source.userId];
                    let systemsData;
                    if (userSession) {
                        ...
                        cs.selectAnswerByText(userSession.getBeforeQuestionId(), event.message.text);
                        ...
                        if (cs.isEnded()) {
                            ...
                            //カルーセルが8枚より上
                                ...
                                const results = cs.getSystems().map((system) => {
                                ...
                                returnMessage = [
                                    ...
                                    await carouselTemplate(results.slice(0, 8), systemsCount, resultId, othersType, imgUrl),
                                ];
                            }
                            else {
                                // 8枚以下
                                ...
                                returnMessage = [
                                    ...
                                    await carouselTemplate(results, systemsCount, resultId, othersType, imgUrl),
                                ];
                            }
                            ...
                            sessions = {
                                ...sessions,
                                [event.source.userId]: new classes_1.Session(cs, cs.selectQuestionFromPriority().id),
                            };
                            returnMessage = [
        ...
        case "follow":
            ...
            break;
        case "postback":
            if (event.postback.data === "start-kumamoto_earthquake" ||
                ...
                const questions = [];
                jsonAnswers["qnas"].forEach((question, i) => {
                    const answers = [];
                    question.answers.forEach((answer, j) => {
                        let syst = {};
                        systems["systems"].forEach((system, k) => {
                            syst = {
                                ...syst,
                                [system]: answer.systems[k].system_answer === "1",
                            };
                        });
                        answers.push(new classes_1.Answer(jsonQuestions.questions[i].answers[j].answer_id, jsonQuestions.questions[i].answers[j].answer_text, syst));
                    });
                    questions.push(new classes_1.Question(jsonQuestions.questions[i].question_id, jsonQuestions.questions[i].question_text, answers));
                });
                // 初期ChatStateの生成
                const cs = new classes_1.ChatState(systems["systems"], questions, selected);
