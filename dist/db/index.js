...
exports.updateUserCount = async (lineId, selected) => {
    const res = await prisma.users.findUnique({
        where: { line_id: lineId },
    });
    if (res) {
...
//制度の利用数ログ
exports.updateUseCount = async (serviceId) => {
...
exports.isLoggedIn = async (lineId) => {
...
exports.getQueryResult = async (resultId) => {
    const res = await prisma.results.findUnique({
        where: { result_id: resultId },
    });
    const seidoType = JSON.parse(res.result_body).result[0].service_id.split("-")[0];
    const img_url = getImageUrl(seidoType);
    if (res) {
        return {
